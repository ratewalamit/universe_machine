...
npp=60
rank= comm.rank
start, end = rank*int(satellites_parent_id.size/npp),int(satellites_parent_id.size/npp)*(rank+1)
if rank==npp-1:
        end=satellites_parent_id.size
...
df_save.write(f"/mnt/home/student/camit/Project_universemachine/DataStore/new/satellites_catalog_sfr_catalog_1.002310_{str(rank).zfill(2)}.fits",overwrite=True)
with MPIPool() as pool:
    if not pool.is_master():
        pool.wait()
        sys.exit(0)
    flist=np.sort(glob("/mnt/home/student/camit/Project_universemachine/DataStore/new/*"))
    for fl,num in enumerate(flist):
        if fl==0:
            new=Table.read(num,format="fits")
        else:
            new=vstack([new,Table.read(num,format="fits")])
    new.write("/mnt/home/student/camit/Project_universemachine/DataStore/new/satellites_catalog_sfr_catalog_1.002310.fits",overwrite=True)   
    pool.close()
