...
idx_orphan=fp[0].values>1e15  #halos with haloid>1e15 are orphan
fp_idx=fp[idx_orphan]

parent_id=fp[0].values
orphan_parent_id=fp_idx[2].values
print("Making tree")
...
for i in tqdm(range(orphan_parent_id.size)):
    idx=my_tree.query_ball_point(orphan_parent_id[i].reshape(-1,1),0.0001)
    if len(idx[0])==0:
        continue
    orphan_info=fp_idx.iloc[i].values
    parent_info=fp.iloc[idx[0]].values[0]
    if parent_info[11]<Mlimit:
        continue
    if (orphan_info[2] != parent_info[0]):
        print("Orphan parent not found in original catalog") 
        break
    #print(orphan_info,parent_info)
    counter+=1
    df_store.add_row(np.concatenate((orphan_info,parent_info)))

idx_parent_not_central=df_store["Parent_UPID"]!=-1
df_save=df_store[~idx_parent_not_central]
print(f"# of orphan satellites= {counter}") 
df_save.write("/mnt/home/student/camit/Project_universemachine/DataStore/orphan_catalog_sfr_catalog_1.002310.fits",overwrite=True)
